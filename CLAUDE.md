...
- **Functional core, imperative shell.** `src/index.ts`, `src/report.ts`,
  ...
  `Bun.WebView` or `Bun.Image`, read the card assets, write a file (the
  card), or spawn a process (the clipboard tool). Everything else is pure functions over plain data: `analyze()` takes
