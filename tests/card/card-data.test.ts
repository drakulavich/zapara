...
// fixture's Sunday: 22 active hours (15 Calm, 3 Warming, 2 Heating, 2 Fried).
...
describe("busy-week card", () => {
  test("the long calm days make it the Marathoner", () => {
    // Shares: conductor 0.31, supervisor 0.19, marathoner 0.77, night owl 0.14 (3 of 22 hours are late).
  ...
  test("the sentence carries the longest streak and the calm share, in bold", () => {
    // Tuesday's calm 10-18 run of prompts reaches 470 min at hour 17 (10:00 to
    // 17:50) -> 7h50m; 15 of 22 hours are Calm -> 68%.
    expect(sentenceText(card.sentence)).toBe("Longest streak 7h50m without a break, 68% of your hours calm.");
    expect(card.sentence.filter((s) => s.strong).map((s) => s.text)).toEqual(["7h50m", "68%"]);
  });
  ...
  test("shares are the fraction of each character's maximum points", () => {
    ...
    // Streak points total 169.7 of a possible 220: Monday's storm no longer
    // inherits the calm morning's streak (an 11-minute gap between the last
    // calm prompt at 11:50 and the storm's first at 12:01), and every calm run
    // now ends at its last prompt rather than the reply three minutes later.
    expect(card.shares.marathoner).toBeCloseTo(0.771, 2);
  ...
  test("spectrum: 15 / 3 / 2 / 2 of 22 hours -> 68 / 14 / 9 / 9, summing to 100", () => {
    // Floors 68/13/9/9 = 99; warming holds the largest remainder (.64) and rounds up.
    expect(card.spectrum).toEqual({ calm: 68, warming: 14, heating: 9, fried: 9 });
  ...
  test("highlights: the owned pair, then the largest remaining norm", () => {
    ...
    expect(card.highlights).toEqual([
      { key: "longestStreak", value: "7h50m", caption: "longest streak" },
