...
describe("busy-week card", () => {
  test("the long calm days make it the Marathoner", () => {
    // Shares: conductor 0.31, supervisor 0.19, marathoner 0.81, night owl 0.14 (3 of 22 hours are late).
  ...
  test("the sentence carries the longest streak and the calm share, in bold", () => {
    // Tuesday's calm 10-18 run reaches 473 min at hour 17 -> 7h53m; 15 of 22 hours are Calm -> 68%.
  ...
  test("shares are the fraction of each character's maximum points", () => {
    ...
    expect(card.shares.marathoner).toBeCloseTo(0.810, 2);
