...
### Added
- `zapara card --copy`: after writing the PNG, put it on the clipboard through the system's own tool (`osascript` on macOS, `wl-copy` or `xclip` on Linux), so one command ends with a picture ready to paste.
